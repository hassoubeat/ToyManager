...
public class ExceptionInterceptor implements Serializable{
    ...
    public Object invoke(InvocationContext context) throws Exception{
        ...
        logger.log(Level.INFO, "START_RUN_METHOD:{0}", targetClassName + "." + targetMethodName);
        
        try {
            // インターセプトするオブジェクトの実行
            ret = context.proceed();
        } catch (Exception ex) {
            logger.log(Level.WARNING, "Exception:{0}" ,targetClassName + "." + targetMethodName + ":" + ex.toString());
        ...
        logger.log(Level.INFO, "END_RUN_METHOD:{0}", targetClassName + "." + targetMethodName);
        long afterTime = System.nanoTime();
        
        logger.log(Level.INFO, "RUN_TIME:{0}", targetClassName + "." + targetMethodName + ":" + (afterTime - beforeTime) + "nsecs.");
